...
#   ARM Adding Buffer POSS  &  Drop Location
                    ...
                    if ((round(yaw,2) >= 2.79 and round(yaw,2)<= 3.14) ) and (round(tool0.transform.translation.y,2) > 0.25):
                        ...
                        self.twist_pub.publish(__twist_msg)
                    elif (round(yaw,2) >= 2.79 and round(yaw,2)<= 3.14) and (round(tool0.transform.translation.y,2) <= 0.25):
                        self.trajactory()
    ...
    while rclpy.ok():
        try:
            ...
            if enftf.variable_a == True:
                ...
                if round(yaw,2) >= -0.17 and round(yaw,2) <=0.52:
                    enftf.trajactory()
